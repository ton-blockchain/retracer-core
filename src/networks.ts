...
const resolveRetraceNetworkConfig = (
  ...
  return {
    ...
    toncenterApiKey:
      toncenterApiKey === undefined || toncenterApiKey.length === 0 ? undefined : toncenterApiKey,
  }
}
...
export function toncenterAddressParam(network: RetraceNetworkConfig, address: Address): string {
  return address.toString({
    testOnly: resolveRetraceNetworkConfig(network).testnet,
  })
}
  ...
  for (let attempt = 1; ; attempt++) {
    ...
    try {
      const response = await fetch(url, {
        method: "POST",
        headers: toncenterHeaders(network, {
